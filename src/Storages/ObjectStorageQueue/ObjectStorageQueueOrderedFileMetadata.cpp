    ...
    LoggerPtr log_)
{
    ...
    return file_path.empty()
        ? ProcessingStateFromKeeper(last_processed_path)
        : ProcessingStateFromKeeper(file_path, last_processed_path, is_failed);
}
...
ObjectStorageQueueOrderedFileMetadata::ProcessingStateFromKeeper::ProcessingStateFromKeeper(
    ...
    , is_processed(path <= last_processed_path;)
{
}
    ...
    LoggerPtr log_)
{
    ...
    for (size_t i = 0; i < buckets_num; ++i)
    {
        ...
        else
        {
            ...
            if (state.last_processed_path.has_value())
                last_processed_file_map[i][""] = std::move(*state.last_processed_path);
        }
    }
